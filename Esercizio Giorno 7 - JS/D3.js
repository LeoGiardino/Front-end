...
let crewMass = 0;
let x = 0;
while (x <= starWarsCharacters.length-1) {
  let massa = 0;
  massa = Number(starWarsCharacters[x].mass);
  crewMass += massa;
  x++;
}
...
let casuale = Math.floor(Math.random() * starWarsCharacters.length);

console.log(starWarsCharacters[casuale])
